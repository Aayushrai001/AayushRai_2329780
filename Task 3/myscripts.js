...
function addReview(movieName, director, rating, releaseDate, reviewData, actor) {
  return addDoc(collection(db, "movies_review"), {
    movie_name: movieName,
    director: director,
    rating: rating,
    release_date: releaseDate,
    review_date: reviewData,
    actor: actor
  });
}
  ...
  unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
      ...
      reviews.push({
        ...
        reviewDate: reviewData.review_date.toDate(),
    ...
    reviews.sort((a, b) => {
      ...
      } else if (sortByField === "reviewDate") {
        return fieldA.getTime() - fieldB.getTime();
      } else {
        // Sort as strings
...
function displayReviews(reviews) {
  ...
  reviews.forEach((review) => {
    ...
    const formattedReviewDate = `${review.reviewDate.getFullYear()}-${review.reviewDate.getMonth() + 1
      }-${review.reviewDate.getDate()}`;
    const tableRow = `
          <tr>
...
function populateEditModal(review) {
  ...
  const formattedReviewDate = review.reviewDate.toISOString().split('T')[0];
  $("#editReviewDate").val(formattedReviewDate); // Input for review date
  ...
  $("#saveChangesButton").on("click", function () {
    ...
    const movieReviewDate = $("#editReviewDate").val().trim(); // Updated review date input
    const movieActor = $("#editActor").val().trim();

    if (movieName === "" || movieRating === "" || movieDirector === "" || movieReleaseDate === "" || movieReviewDate === "" || movieActor === "") {
      $("#editError").show();
      return;
    }
    ...
    const data = {
      ...
      review_date: new Date(movieReviewDate), // Updated review date
      actor: movieActor
    };
...
$("#addReviewButton").click(function () {
  ...
  const movieReviewDate = $("#addReviewDate").val().trim();
